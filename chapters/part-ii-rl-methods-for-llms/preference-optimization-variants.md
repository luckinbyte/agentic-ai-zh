...
$$
\mathcal{L}_{\text{KTO}} = \mathbb{E}_{y_w}\big[\lambda_w\,(1 - v(x, y_w))\big] + \mathbb{E}_{y_l}\big[\lambda_l \cdot v(x, y_l)\big] \tag{8.1}
$$
...
$$
\mathcal{L}_{\text{IPO}} = \mathbb{E}\left[ \left( \log \frac{\pi_\theta(y_w \mid x)}{\pi_{\text{ref}}(y_w \mid x)} - \log \frac{\pi_\theta(y_l \mid x)}{\pi_{\text{ref}}(y_l \mid x)} - \frac{1}{2\beta} \right)^2 \right] \tag{8.2}
$$
...
\;-\; \lambda \cdot \underbrace{\log \sigma\!\left( \log \frac{\text{odds}_\theta(y_w \mid x)}{\text{odds}_\theta(y_l \mid x)} \right)}_{\text{通过赔率比进行偏好对齐}} \tag{8.3}
$$
...
$$
y^{*} = \arg\max_{y_i \sim \pi_\theta(\cdot \mid x)}\; r_\phi(x, y_i) \tag{8.4}
$$
