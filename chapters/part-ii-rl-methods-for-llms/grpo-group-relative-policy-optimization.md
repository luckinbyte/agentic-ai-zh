...
$$
\hat{A}_i = \frac{r_i - \mu_G}{\sigma_G}, \qquad \mathcal{L} = \mathbb{E}\left[ \min\left( r_t(\theta)\hat{A}_i,\ \text{clip}\big(r_t(\theta),\, 1 \pm \epsilon\big)\hat{A}_i \right) \right] - \beta\, D_{KL}\!\left[\pi_\theta \,\|\, \pi_{ref}\right] \tag{7.1}
$$
...
$$
\hat{A}_i^{GOPO} = f\!\left(\frac{\text{rank}(o_i)}{N}\right) \tag{7.2}
$$
