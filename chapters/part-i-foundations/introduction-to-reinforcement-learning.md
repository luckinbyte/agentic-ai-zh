...
$$G_t = \sum_{k=0}^{\infty} \gamma^k r_{t+k} = r_t + \gamma r_{t+1} + \gamma^2 r_{t+2} + \cdots \quad (3.1)$$

**价值函数(Value Function，在策略 $\pi$ 下从状态 $s$ 出发的期望回报)**：

$$V^\pi(s) = \mathbb{E}_\pi \left[ G_t \mid s_t = s \right] = \mathbb{E}_\pi \left[ \sum_{k=0}^{\infty} \gamma^k r_{t+k} \;\middle|\; s_t = s \right] \quad (3.2)$$

**动作-价值函数(Action-Value Function，在状态 $s$ 采取动作 $a$ 后再遵循 $\pi$ 的期望回报)**：

$$Q^\pi(s, a) = \mathbb{E}_\pi \left[ G_t \mid s_t = s, a_t = a \right] \quad (3.3)$$
...
$$V^\pi(s) = \sum_{a} \pi(a \mid s) \sum_{s'} P(s' \mid s, a) \left[ R(s, a, s') + \gamma V^\pi(s') \right] \quad (3.5)$$

$$Q^\pi(s, a) = \sum_{s'} P(s' \mid s, a) \left[ R(s, a, s') + \gamma \sum_{a'} \pi(a' \mid s') Q^\pi(s', a') \right] \quad (3.6)$$

**最优策略与贝尔曼最优性(Bellman Optimality)**：最优策略 $\pi^*$ 满足：

$$V^*(s) = \max_{a} \sum_{s'} P(s' \mid s, a) \left[ R(s, a, s') + \gamma V^*(s') \right] \quad (3.7)$$

$$Q^*(s, a) = \sum_{s'} P(s' \mid s, a) \left[ R(s, a, s') + \gamma \max_{a'} Q^*(s', a') \right] \quad (3.8)$$
...
$$\text{TD 误差} = \text{TD 目标} - \text{旧估计} \quad (3.10)$$
...
$$G_t^{(n)} = r_t + \gamma r_{t+1} + \cdots + \gamma^{n-1} r_{t+n-1} + \gamma^n V(s_{t+n}) \quad (3.12)$$
...
$$Q(s_t, a_t) \leftarrow Q(s_t, a_t) + \alpha \left[ r_t + \gamma Q(s_{t+1}, a_{t+1}) - Q(s_t, a_t) \right] \quad (3.14)$$
...
$$\mathcal{L}(\theta) = \mathbb{E}_{(s,a,r,s') \sim \mathcal{B}} \left[ \left( r + \gamma \max_{a'} \bar{Q}_\theta(s', a') - Q_\theta(s, a) \right)^2 \right] \quad (3.15)$$
...
$$\nabla_\theta J(\theta) = \mathbb{E}_{\pi_\theta} \left[ \sum_{t=0}^{T} \nabla_\theta \log \pi_\theta(a_t \mid s_t) \cdot G_t \right] \quad (3.19)$$
...
$$\nabla_\theta J(\theta) = \mathbb{E}_{\pi_\theta} \left[ \sum_{t=0}^{T} \nabla_\theta \log \pi_\theta(a_t \mid s_t) \cdot (G_t - b(s_t)) \right] \quad (3.20)$$
...
$$\nabla_\theta J = \mathbb{E} \left[ \nabla_\theta \log \pi_\theta(a_t \mid s_t) \cdot \hat{A}_t \right], \quad \hat{A}_t = r_t + \gamma V_\phi(s_{t+1}) - V_\phi(s_t) \quad (3.21)$$

**Critic 更新**(最小化 TD 误差)：

$$\mathcal{L}_{\text{critic}} = \mathbb{E} \left[ \left( r_t + \gamma V_\phi(s_{t+1}) - V_\phi(s_t) \right)^2 \right] \quad (3.22)$$
...
$$\hat{A}_t^{\text{GAE}(\gamma, 0)} = \delta V_t = r_t + \gamma V_\theta(s_{t+1}) - V_\theta(s_t) \quad (3.24)$$
