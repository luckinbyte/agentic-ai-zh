...
$$
P_\text{parametric}(a \mid q) = P_{M_\theta}(a \mid q) \quad (16.1)
$$

$$
P_\text{RAG}(a \mid q, D) = \sum_{d \in D} P_{M_\theta}(a \mid q, d)\, P_\text{ret}(d \mid q, D) \quad (16.2)
$$
...
$$
\text{sim}(\mathbf{q}, e_i) = \frac{\mathbf{q} \cdot e_i}{\lVert \mathbf{q} \rVert\, \lVert e_i \rVert} \quad (16.3)
$$
...
$$
\mathcal{L}_\text{DPR} = -\frac{1}{B} \sum_{i=1}^{B} \log \frac{\exp\left(E_Q(q_i)^\top E_P(p_i^+) / \tau\right)}{\sum_{j=1}^{B} \exp\left(E_Q(q_i)^\top E_P(p_j) / \tau\right)} \quad (16.6)
$$
...
$$
\text{RRF}(d) = \sum_{r \in R} \frac{1}{k + \text{rank}_r(d)} \quad (16.8)
$$
...
$$
w_t(x) = \log\left(1 + \text{ReLU}\left(\max_{i \in [1, n]} H_i[t]\right)\right) \quad (16.9)
$$
...
$$
s(q, d) = \sum_{t \in V} w_q^t \cdot w_d^t \quad (16.10)
$$
...
$$
\mathcal{L} = \mathcal{L}_\text{contrastive} + \lambda_q \lVert w_q \rVert_1 + \lambda_d \lVert w_d \rVert_1 \quad (16.11)
$$
...
$$
\mathcal{L}_\text{distill} = \text{KL}\left(\sigma(s_\text{student}) \,\|\, \sigma(s_\text{teacher})\right) \quad (16.12)
$$

2. **分离的查询/文档编码器:** SPLADEv2 对查询与文档使用不同的稀疏目标。查询被鼓励更稀疏(查找更快),而文档可以略稠密(离线预计算):

$$
\lambda_q > \lambda_d \quad (\text{例如 } \lambda_q = 3 \times 10^{-4},\ \lambda_d = 1 \times 10^{-4}) \quad (16.13)
$$

3. **FLOPS 正则化:** SPLADEv2 不再使用简单 L1,而是引入一个 FLOPS 感知的正则项,直接惩罚预期的检索开销:

$$
\mathcal{L}_\text{FLOPS} = \sum_{t \in V} (a_q^t)^2 + \sum_{t \in V} (a_d^t)^2 \quad (16.14)
$$
...
$$
s(q, d) = \sum_{i \in |q|} \max_{j \in |d|} q_i^\top d_j \quad (16.15)
$$
...
$$
q_i = \text{Linear}(E_Q(q)_i) \in \mathbb{R}^{128}, \quad i = 1, \dots, |q| \quad (16.16)
$$

$$
d_j = \text{Linear}(E_D(d)_j) \in \mathbb{R}^{128}, \quad j = 1, \dots, |d| \quad (16.17)
$$
...
$$
\hat{d} = \text{LLM}(q), \quad e_\text{query} = f_\phi(\hat{d}) \quad (16.20)
$$
...
$$
\text{Recall@K} = \frac{|R_K \cap R^*|}{|R^*|} \quad (16.22)
$$

**Precision@K。**

$$
\text{Precision@K} = \frac{|R_K \cap R^*|}{K} \quad (16.23)
$$
...
$$
\text{Faithfulness} = \frac{\text{被上下文支持的论断数}}{\text{答案中总论断数}} \quad (16.26)
$$

**答案相关性(Answer Relevance)。** 衡量答案是否针对了问题。通过从答案生成问题并测量与原始查询的相似度来计算:

$$
\text{AnswerRelevance} = \frac{1}{N} \sum_{i=1}^{N} \cos\left(E(q),\, E(\hat{q}_i)\right) \quad (16.27)
$$
...
$$
\text{ContextPrecision@K} = \frac{1}{K} \sum_{k=1}^{K} \text{Precision@k} \cdot \mathbb{1}[\text{doc}_k \text{ is relevant}] \quad (16.28)
$$

$$
\text{ContextRecall} = \frac{\text{可归因到上下文的真值论断数}}{\text{总真值论断数}} \quad (16.29)
$$
...
$$
\mathcal{L}_\text{RAFT} = -\mathbb{E}_{(q, a, d^*, \{d_i^-\})} \left[ \log P_\theta\left( \text{CoT}(d^*) \oplus a \,\big|\, q, d^*, \{d_i^-\} \right) \right] \quad (16.30)
$$
...
$$
\nabla_\theta \mathcal{L} = \nabla_\theta \left[ -\log \sum_{d \in D} P_\theta(a \mid q, d) \cdot P_\phi(d \mid q) \right] \quad (16.31)
$$
