...
$$
e_t = (S_t,\ A_t,\ R_t,\ S_{t+1}) \quad (12.1)
$$
...
$$
A_t = \{\text{text}_{\text{reasoning}},\ \text{json}_{\text{tool\_call}}\} \quad (12.2)
$$
...
$$
\text{Critique} \leftarrow \text{LLM}(S_{\text{failed}},\ A_{\text{failed}},\ R=0) \quad (12.3)
$$
...
$$
E_{\text{retrieved}} = \arg\max_{e \in \mathcal{B}}\ \text{sim}\big(\text{Embed}(G_{\text{new}}),\ \text{Embed}(e)\big) \quad (12.4)
$$
...
$$
\theta_{k+1} = \arg\min_\theta -\sum_{(x, z, y) \in D_k^+} \log \pi_\theta(z, y \mid x) \quad (12.5)
$$
...
$$
a_{t+1} \sim \pi(\cdot \mid \text{task},\ M,\ \text{current\_state}) \quad (12.7)
$$
...
$$
(\tau_w,\ \tau_l)\ \ \text{其中}\ \ r(\tau_w) > r(\tau_l) \quad (12.9)
$$
...
$$
\text{skill}_i = \text{LLM}(\text{task}_i,\ \text{environment\_docs},\ \text{error\_feedback}) \quad (12.11)
$$
...
$$
\text{solution} = \text{LLM}(\text{new\_task},\ \text{retrieve}(\text{skill\_library},\ k=5)) \quad (12.12)
$$
...
$$
o_t = [\text{system\_prompt};\ \text{user\_intent};\ \text{tool\_history}_{1:t-1};\ \text{current\_result}_t] \quad (12.15)
$$
...
$$
R_{\text{step}}(t) = \underbrace{R_{\text{sub-goal}}(t)}_{\text{当前子目标是否成功?}} + \underbrace{\gamma^{T-t} R_{\text{terminal}}}_{\text{折扣后的最终奖励}} + \underbrace{r_{\text{intermediate}}(t)}_{\text{逐步 API 成功/失败}} \quad (12.17)
$$
...
$$
A_i = \frac{r(o_i) - \frac{1}{N}\sum_{j=1}^{N} r(o_j)}{\text{std}\big(r(o_1),\ \dots,\ r(o_N)\big)} \quad (12.18)
$$

带 KL 正则化的 GRPO 目标:

$$
\mathcal{L}_{\text{GRPO}}(\theta) = \frac{1}{N} \sum_{i=1}^{N} \left[ \min\!\left( \frac{\pi_\theta(o_i \mid q)}{\pi_{\theta_{\text{old}}}(o_i \mid q)} A_i,\ \ \text{clip}\!\left(\frac{\pi_\theta(o_i \mid q)}{\pi_{\theta_{\text{old}}}(o_i \mid q)},\ 1-\epsilon,\ 1+\epsilon\right) A_i \right) \right] - \beta\, D_{KL}\!\left(\pi_\theta \,\|\, \pi_{\text{ref}}\right) \quad (12.19)
$$
